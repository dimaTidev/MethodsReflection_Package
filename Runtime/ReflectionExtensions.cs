...
{
    public static List<FieldInfo> GetFields(this MonoBehaviour mb, Type type, BindingFlags flags)
    {
        if (mb == null)
        {
            MonoBehaviour.print("No monoBehaviour in GetMethods");
            return null;
        }

        return mb.GetType().GetFields(flags)
            .Where(m => m.FieldType == type)//.Select(m => m)
            .ToList();
    }
